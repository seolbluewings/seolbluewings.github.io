...
set.seed(2081311133)
library(mvtnorm)
WC=read.csv("C://Users/WC_data.csv",header=T)
...
for(g in 1:G){
  ...
  zg[[g]]=as.vector(scale(x[WC$group==g,2]))
  mg[g]=length(yg[[g]])
}
...
lambda_list=list()
sige_list=list();sigez_list=list()
...
for(r in 1:R){
  ...
  beta0=rep(0,2*dim(Xg[[1]])[2]); Beta0=5*diag(1,2*dim(Xg[[1]])[2])
  ...
  beta_mat=matrix(0,ncol=2*dim(Xg[[1]])[2],nrow=iter) #save for beta
  ...
  for(t in 2:iter){
    ...
    for(g in 1:G){
      ...
      for(v in 1:mg[g]){
        ...
        V=(sige_mat[t-1,]-sigez_mat[t-1,]^2)*diag(1,mg[g])
    ...
    for(g in 1:G){
      ...
      XX=cbind(Xg[[g]],Wg[[g]]%*%as.matrix(Xg[[g]]))
    ...
    pp_sig=1
    for(g in 1:G){
      V1=(sige_1-sigez_1^2)*diag(1,mg[g])
      V2=(sige_mat[t-1,]-sigez_mat[t-1,]^2)*diag(1,mg[g])
      SS=diag(1,mg[g])-lambda_mat[t,]*Wg[[g]]
      XX=cbind(Xg[[g]],Wg[[g]]%*%Xg[[g]])
      ep=SS%*%yg[[g]]-XX%*%beta_mat[t-1,]
    ...
    for(g in 1:G){
      Rg=(Alpha0^(-1)+(sige_mat[t,]-sigez_mat[t,]^2)^(-1)*t(rep(1,mg[g]))%*%rep(1,mg[g]))^(-1)
      SS=diag(1,mg[g])-lambda_mat[t,]*Wg[[g]]
      YY=SS%*%yg[[g]]-sigez_mat[t,]*Zlist[[g]]
      XX=cbind(Xg[[g]],Wg[[g]]%*%Xg[[g]])
      alpha_hat=Rg%*%(Alpha0^(-1)*alpha0+(sige_mat[t,]-sigez_mat[t,]^2)^(-1)*rep(1,mg[g])%*%(YY-XX%*%beta_mat[t,]))
      alpha_mat[t,g]=rnorm(1,alpha_hat,sqrt(Rg))
    }
    
    end_time=Sys.time()
    ###print result
    cat("Own Xg =", round(beta_mat[t,1],3),"\n")
    cat("Own Goal =", round(beta_mat[t,2],3),"\n")
    cat("Own Shot =", round(beta_mat[t,3],3),"\n")
    cat("Own Shot Assitst =", round(beta_mat[t,4],3),"\n")
    cat("Own Cross =", round(beta_mat[t,5],3),"\n")
    cat("Own Clearance =", round(beta_mat[t,6],3),"\n")
    cat("Own Dribble =", round(beta_mat[t,7],3),"\n")
    cat("Own Duel =", round(beta_mat[t,8],3),"\n")
    cat("Own Pressure =", round(beta_mat[t,9],3),"\n")
    cat("Peer Xg =", round(beta_mat[t,10],3),"\n")
    cat("Peer Goal =", round(beta_mat[t,11],3),"\n")
    cat("Peer Shot =", round(beta_mat[t,12],3),"\n")
    cat("Peer Shot Assist =", round(beta_mat[t,13],3),"\n")
    cat("Peer Cross =", round(beta_mat[t,14],3),"\n")
    cat("Peer Clearance =", round(beta_mat[t,15],3),"\n")
    cat("Peer Dribble =", round(beta_mat[t,16],3),"\n")
    cat("Peer Duel =", round(beta_mat[t,17],3),"\n")
    cat("Peer Pressure =", round(beta_mat[t,18],3),"\n")
