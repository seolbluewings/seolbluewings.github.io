...
for(r in 1:R){
  ...
  sigu_mat[1,]=0.5
  beta_mat[1,]=c(0.3,0.3);mu_mat[1,]=rep(0.5,n)
  ...
  for(t in 2:iter){
    ...
    for(g in 1:G){
      ...
      for(v in 1:n){
        ...
        a=rep(1,n)*alpha_mat[t-1,g]
        like_Y1=exp(-0.5*t(ep-d1-d3-a)%*%solve(V)%*%(ep-d1-d3-a))
        like_Y2=exp(-0.5*t(ep-d2-d3-a)%*%solve(V)%*%(ep-d2-d3-a))
      ...
      mu=MM%*%delta_mat[t-1,]
      a=rep(1,n)*alpha_mat[t-1,g]
      like_1=det(S_1)*exp(-0.5*t(ep_1-del-mu-a)%*%solve(V)%*%(ep_1-del-mu-a))
      like_2=det(S_2)*exp(-0.5*t(ep_2-del-mu-a)%*%solve(V)%*%(ep_2-del-mu-a))
    ...
    for(g in 1:G){
      ...
      MM=cbind(mu_mat[t-1,],Wg[[g]]%*%mu_mat[t-1,])
      ZZ=cbind(Z_mat[g,],Wg[[g]]%*%Z_mat[g,])
      YY=SS%*%yg[[g]]-ZZ%*%delta_mat[t-1,]-MM%*%delta_mat[t-1,]-rep(1,n)*alpha_mat[t-1,g]
      XX=cbind(Xg[[g]],Wg[[g]]%*%Xg[[g]])
    ...
    for(g in 1:G){
      ...
      ZZ=cbind(Z_mat[g,],Wg[[g]]%*%Z_mat[g,])
      MM=cbind(mu_mat[t-1,],Wg[[g]]%*%mu_mat[t-1,])
      ep=SS%*%yg[[g]]-XX%*%beta_mat[t,]-rep(1,n)*alpha_mat[t-1,g]
      del1=ZZ%*%t(delta_1)
      del2=ZZ%*%delta_mat[t-1,]
      mu1=MM%*%t(delta_1)
      mu2=MM%*%delta_mat[t-1,]
      a=rep(1,n)*alpha_mat[t-1,g]
      like_1=exp(-0.5*t(ep-del1-mu1-a)%*%solve(V)%*%(ep-del1-mu1-a))
      like_2=exp(-0.5*t(ep-del2-mu2-a)%*%solve(V)%*%(ep-del2-mu2-a))
    ...
    for(g in 1:G){
      ...
      del=ZZ%*%delta_mat[t,]
      mu=MM%*%delta_mat[t,]
      ug=ep-del-mu-rep(1,n)*alpha_mat[t-1,g]
      UG=UG+t(ug)%*%ug
    }
    ...
    Rg=(Alpha0^(-1)+(sigu_mat[t,])^(-1)*t(rep(1,n))%*%rep(1,n))^(-1)
    for(g in 1:G){
      SS=diag(1,n)-lambda_mat[t,]*Wg[[g]]
      ZZ=cbind(Z_mat[g,],Wg[[g]]%*%Z_mat[g,])
      MM=cbind(mu_mat[t,],Wg[[g]]%*%mu_mat[t,])
      ...
      XX=cbind(Xg[[g]],Wg[[g]]%*%Xg[[g]])
      al_hat=Rg*(Alpha0^(-1)*alpha0+rep(1,n)%*%solve(sigu_mat[t,]*diag(1,n))%*%(YY-XX%*%beta_mat[t,]))
      alpha_mat[t,g]=rnorm(1,al_hat,sqrt(Rg))
    }
