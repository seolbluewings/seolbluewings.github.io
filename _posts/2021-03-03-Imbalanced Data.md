...
5번의 과정에서 선택된 데이터는 Class를 결정짓는 경계에 위치한 데이터이기 때문에 Danger Instance라고 부른다.
...
Borderline SMOTE는 대표적으로 2가지 방법으로 작동한다. Borderline SMOTE 1이라 불리는 방식은 바로 앞서 소개한 방식과 동일하다. 최종 6번째 단계에서 합성 데이터를 생성할 때, $$S_{min}$$에 포함된 데이터만 따져서 합성 데이터를 만든다.

Borderline SMOTE 2라 불리는 방식은 이와 조금 다르다. 합성 데이터를 생성하기 위한 데이터 포인트 선별 과정(즉 1~5단계)은 동일하다. 그러나 합성 데이터를 만들어낼 점을 선택한 이후에는 $$S_{min}$$ 뿐만 아니라 $$S_{maj}$$ 도 같이 고려하여 K-NN 을 찾고 $$S_{maj}$$까지 포함해서 임의의 한 점을 선택한다. 단, 이 때 임의로 선택한 데이터 포인트가 $$S_{maj}$$에 속하는 데이터 포인트라면, 가중치를 (0,1) 사이의 값이 아닌 (0,0.5) 사이의 값으로 한정하여 합성 데이터가 그나마 $$S_{min}$$에 가까이 위치하도록 설정해준다.


SMOTE와 Borderline SMOTE에 대한 간략한 코드는 다음의 [링크](https://github.com/seolbluewings/Python/blob/master/14.%20SMOTE.ipynb)에서 확인 가능합니다.
