...
만약 제약 조건 $$g(\mathbf{X}) \geq 0$$ 에서 함수 $$f(\mathbf{X})$$ 에 대한 최소화를 진행한다고 가정하면, $$\lambda \geq 0$$ 조건에서 라그랑주 함수 $$L(\mathbf{X},\lambda) \equiv f(\mathbf{X}) - \lambda g(\mathbf{X})$$ 를 $$\mathbf{X}$$에 대해 최소화시키는 것으로 변형시킬 수 있다. 
