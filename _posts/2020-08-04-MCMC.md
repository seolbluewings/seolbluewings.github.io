...
$$ p(\theta\mid\mathbf{X}) \prop p(\theta)p(\mathbf{X}\mid\theta) $$
...
최적의 transition probability $$T(\theta^{(t)},\theta^{(t+1)}$$ 를 생성한 후, 이 transition probability를 활용한 Markov-Chain을 시행한다.
