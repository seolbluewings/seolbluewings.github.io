...
&\propto \prod_{t=1}^{T} \frac{ \Gamma\left(\beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t) \right) }{ \Gamma\left( \sum_{m=1}^{M} \left( \beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t) \right) \right) } \prod_{t=1}^{T}\Gamma\left( \alpha_{t}+\sum_{n=1}^{N_{d}}I(z_{n}^{(d)}=t)  \right) \nonumber \quad \text{Constant Term 제거}
\end{align}
$$
