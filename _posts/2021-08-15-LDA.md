...
LDA에서 Collapsed Gibbs Sampler를 사용하는 이유 수식을 간결하게 만들기 위함이다.
...
$$ p(z,\phi,\theta \vert w) \propto \prod_{d=1}^{D}\prod_{n=1}^{N_{d}}p(w_{n}^{(d)}\vert z_{n}^{(d)},\phi)\prod_{d=1}^{D}\prod_{n=1}^{N_{d}}p(z_{n}^{(d)}\vert\theta)\prod_{t=1}^{T}p(\phi^{(t)}\vert \beta)\prod_{d=1}^{D}p(\theta^{(d)}\vert \alpha) $$
