...
이 수식을 한 Step 더 들어가면 다음과 같이 표현이 가능할 것이다. Sampling Step에서 d번째 문서의 n번째 단어에 대해 토픽 t를 할당하기 위해서 나머지 단어의 토픽 할당은 그대로 유지한 상태여야 한다. 따라서 토픽에 대한 연산을 수행할 때, t번째 토픽에 대해서는 제외하고 계산이 이루어지도록 설정하고 그 이후 강제로 토픽 t를 할당한 부분을 곱해주는 방식을 취한다. 그래서 뒤에 곱해지는 파트에 +1 처리가 진행된다.

$$
\begin{align}
p(z_{n}^{(d)}=t, z_{-n}^{(d)},\mathbf{w},\alpha,\beta) &\propto \prod_{{i=1}^{T},i \neq t} \frac{ \Gamma\left(\beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=i) \right) }{ \Gamma\left( \sum_{m=1}^{M} \left( \beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=i) \right) \right) } \times \prod_{{i=1}^{T},i \neq t}\Gamma\left( \alpha_{t}+\sum_{n=1}^{N_{d}}I(z_{n}^{(d)}=i)  \right) \nonumber \\
&\times \frac{ \Gamma\left(\beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t+1) \right) }{ \Gamma\left( \sum_{m=1}^{M} \left( \beta_{m}+1+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t) \right) \right) } \times \prod_{{i=1}^{T},i \neq t}\Gamma\left( \alpha_{t}+1+\sum_{n=1}^{N_{d}}I(z_{n}^{(d)}=t)  \right) \nonumber
\end{algin}
$$

그런데 $$ \Gamma(x+1)=x\Gamma(x) $$ 인 Gamma Function의 특징을 이용하면, 다음과 같이 수식의 뒷부분을 변경할 수 있다.

$$
\begin{align}
\frac{ \Gamma\left(\beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t+1) \right) }{ \Gamma\left( \sum_{m=1}^{M} \left( \beta_{m}+1+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t) \right) \right) } \times \prod_{{i=1}^{T},i \neq t}\Gamma\left( \alpha_{t}+1+\sum_{n=1}^{N_{d}}I(z_{n}^{(d)}=t)  \right) \nonumber \\
&= \frac{ \Gamma\left(\beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t) \right) }{ \Gamma\left( \sum_{m=1}^{M} \left( \beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t) \right) \right) } \times \prod_{{i=1}^{T},i \neq t}\Gamma\left( \alpha_{t}+\sum_{n=1}^{N_{d}}I(z_{n}^{(d)}=t)  \right) \nonumber \\
&\times \frac{ \beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t)  }{  \sum_{m=1}^{M} \left( \beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t) \right) } \times \left( \alpha_{t}+\sum_{n=1}^{N_{d}}I(z_{n}^{(d)}=t) \right) \nonumber
\end{align}
$$

이렇게 해서 구한 $$ \frac{ \beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t)  }{  \sum_{m=1}^{M} \left( \beta_{m}+ \sum_{d=1}^{D}\sum_{n=1}^{N_{d}}I(w_{n}^{(d)}=m)I(z_{n}^{(d)}=t) \right) } \times \left( \alpha_{t}+\sum_{n=1}^{N_{d}}I(z_{n}^{(d)}=t) \right) $$ 이 Gibbs Sampler 과정에 필요한 단순한(?) Term이 된다.
