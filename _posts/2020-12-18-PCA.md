...
$$
\begin{align}
\mathbf{a} &= c_{1}\gamma_{1}+c_{2}\gamma_{2}+...+c_{D}\gamma_{D} \quad \mathbf{a}=\Gamma C \nonumber \\
...
&= C^{T}\Gamma^{T}(\Gamma\Lambda\Gamma^{T})\Gamma C = C^{T}\Lambda C = \sum_{i=1}^{D}]\lambda_{i}c_{i}^{2}
\end{align}
$$
